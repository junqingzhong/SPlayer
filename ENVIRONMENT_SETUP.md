...
- **Node.js**: Version `>=20` is required (as specified in `package.json`).
  - _Note_: The recent updates (including proxy settings) were developed in an environment with Node.js v18.19.1. While the application builds and runs, full compatibility (especially regarding TypeScript type checking for `electron-store`) is best achieved with Node.js v20 or higher.
