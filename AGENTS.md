...
- **规划/总结文档**: 任务过程中生成的规划文档（如 `name_of_task.md`）和总结文档（如 `test_lrc.md`），处理流程如下：
  - **默认行为**：当 `pnpm lint` 与 `pnpm build` 验证无问题（0 错误 0 警告）后，必须自动删除，严禁推送到代码库。
  - **保留例外**：若用户明确要求保留，需在交付时再次与用户确认保留路径与原因。
  - **强制清理时机**：每次任务交付前，必须确认仓库根目录及 `src/` 下无遗留的 `.md` 规划/测试总结文档。
