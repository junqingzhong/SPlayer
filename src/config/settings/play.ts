import { computed, ref, h, watch, onMounted, VNodeChild } from "vue";
...
export const usePlaySettings = (): SettingGroup[] => {
  ...
  const songLevelOptions = computed(() => {
    ...
    if (settingStore.disableAiAudio) {
      return options.filter((option) => {
        ...
        return !AI_AUDIO_LEVELS.includes(option.value as any);
      });
    }
    return options;
  });
  ...
  watch(
    ...
    (val) => {
      if (!val) return;
      if (AI_AUDIO_LEVELS.includes(settingStore.songLevel as any)) {
  ...
  return [
    {
      ...
      items: [
        ...
        },
        {
          ...
          description: () =>
            h("div", [
              ...
              // 简化一点，直接用 text 也可以，不过原代码有颜色
            ]),
          // 由于 description 在 SettingItemRenderer 是用 component :is 渲染的，
          // 或者 v-html。如果返回 VNode，需要 component :is
          // 注意：SettingItemRenderer 实现里用了 typeof item.description === 'function' ? :is="item.description"
          // 这里的 description 是一个 computed (隐式)，或者直接函数。
          // 这里的 description 用于显示 engineTip
          ...
          value: computed({
            ...
            set: (v) => handleAudioEngineSelect(v), // 直接调用处理函数
          }),
        },
        {
          key: "playSongDemo",
        ...
        },
        {
          ...
          componentProps: {
            renderOption, // 通用 renderOption
          },
          // 注意：我们之前的 implementation 在 SettingItemRenderer.vue 里直接用了 v-bind="item.componentProps"
          // disabled 已经在 item.disabled 处理了，但也可以在 componentProps 里覆盖 n-select 的 props
          // 这里我们把 disabled 放到 item config 顶层更好
          disabled: computed(
    ...
    },
    {
      title: "音乐解锁",
      tags: [{ text: "Beta", type: "warning" }],
      // Electron Only (Beta tag handled in title rendering? No, SettingGroup title is string.
      // We might need to handle Beta tag in UniversalSetting.vue title rendering if we want it strictly same.
      // For now ignore group title tag, or add it to string)
      items: [
        {
          key: "useSongUnlock",
          label: "音乐解锁",
