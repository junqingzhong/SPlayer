...
class DiscordRpcManager {
  ...
  private currentMetadata: DiscordMetadataParam | null = null;
  private currentStatus: "playing" | "paused" = "paused";
  private currentTimeline: DiscordTimelineParam = { currentTime: 0, totalTime: 0 };
  private lastSentEndTimestamp: number | undefined;
   ...
   */
  public updateMetadata(metadata: DiscordMetadataParam) {
   ...
   */
  public updateTimeline(timeline: DiscordTimelineParam) {
  ...
  private updateActivity() {
    ...
    const { songName, authorName, albumName, originalCoverUrl, ncmId, duration } =
    ...
    if (this.currentStatus === "playing" && duration) {
      ...
      if (this.lastSentEndTimestamp) {
        const diff = Math.abs(endTimestamp - this.lastSentEndTimestamp);
        if (diff < 2) {
          // Less than 2 seconds difference, likely just normal playback progression or minor drift
          // Don't update to avoid rate limits and flickering
          return;
        }
      }
      this.lastSentEndTimestamp = endTimestamp;
      ...
      this.lastSentEndTimestamp = undefined; // Reset on pause
    } else {
      this.lastSentEndTimestamp = undefined;
    }
  ...
  private processCoverUrl(url?: string): string {
    ...
    try {
      ...
      if (urlObj.hostname === "music.126.net" || urlObj.hostname.endsWith(".music.126.net")) {
